...
3. **Environment Configuration**
   ...
   # Error reporting - shared by both the production and development bot
   ERROR_GUILD_ID=your_error_guild_id
   ...
   > `ERROR_GUILD_ID` / `ERROR_CHANNEL_ID` are where `Logger.error` sends error
   > embeds. Both bots report to the same channel, so **each bot must be a
   > member of that guild** with permission to send messages there. If either
   > variable is unset, Discord error reporting is disabled and errors are only
...
_For support or questions, please contact me or create an issue in this repository._

_This README has mostly been generated by AI because I was too lazy to write it myself._
